...
const getWeatherData = async (city)=>{
    ...
    const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiId}&units=metric&lang=sp`);//nose si eso de los headers es necesario
...
const displayBackgroundImage = (obj)=>{
    ...
    let dateRosario = new Date(obj.dt*1000).toLocaleString("es-AR",{timeStyle:"short",dateStyle:"long"});//dos parametros, el primero que es lo que entiendo por local, el segundo un objeto que especifique funciones de como quiero que se despliegue la fecha
    
    //manipulo el DOM para incluir la hora
    date.textContent = `Dia y Hora ${dateRosario}`;
